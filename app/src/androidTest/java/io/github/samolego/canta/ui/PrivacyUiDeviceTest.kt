...
class PrivacyUiDeviceTest {
    ...
    @Test fun actualPermissionAndBackgroundValuesAndWorkProfileAreVisible() = runBlocking {
        ...
        compose.waitUntil(90_000) { compose.onAllNodesWithText("CAMERA · granted:", substring = true).fetchSemanticsNodes().isNotEmpty() }
        compose.onNodeWithText("Profile: user 10").assertIsDisplayed()
        compose.onNodeWithText("CAMERA · granted:", substring = true).performScrollTo().assertIsDisplayed()
        ...
        compose.onNodeWithText("Desired block:", substring = true).performScrollTo().assertIsDisplayed()
        Unit
    }
    @Test fun lockdownPresetRoundtripAndApplyOnlyAffectCapturedWorkProfile() = runBlocking {
        ...
        val original = CantaPresetData("Privacy test", "Work only", 1, emptySet(), uuid = "privacy-test",
        ...
        val record = services.history.records.first().last { it.packageName == pkg && it.action == "metered" && it.userId == 10 }
